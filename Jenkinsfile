...
def slack_send_summary(testlog, test_failed) {
  ...
  def title_string     =  "\"title\":\":white_check_mark: Deployment API Integration Tests for ${env.BRANCH_NAME} Succeeded!\"";
  ...
  if (test_failed == true) {
    ...
    title_string =  "\"title\":\":boom: Deployment API Integration Tests for ${env.BRANCH_NAME} Failed!\"";
  }
...
def slack_send_testlog(testlog) {
  withCredentials([string(credentialsId: 'slack-file-poster-token', variable: 'SLACK_TOKEN')]) {
    ...
    def requestBody = [
      "token=${SLACK_TOKEN}",
      "content=${testlog}",
      "filename=deployment_api_integration_tests.txt",
    ...
    stage('Prepare') {
      steps {
        script {
          ...
          db_image       = docker.build("deployment_api_test_db_image:${image_tag}", '--file _integration_tests/Dockerfile.database _integration_tests');
          server_image   = docker.build("deployment_api_test_server_image:${image_tag}", '--no-cache --file _integration_tests/Dockerfile.tests _integration_tests');
